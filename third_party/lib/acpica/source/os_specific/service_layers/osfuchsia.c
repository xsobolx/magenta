...
struct acpi_pci_table magenta_pci_tbl = { NULL, 0, false, false };
...
// Allow acpisvc to pull the information we probed earlier rather than parsing
// everything again.
struct acpi_pci_table* get_magenta_pci_table(void) {
    return &magenta_pci_tbl;
}
...
static ACPI_STATUS acpi_pci_ecam_cfg_rw(ACPI_PCI_ID *PciId, uint32_t reg,
    ...
    if (offset >= magenta_pci_tbl.ecam_size) {
        printf("ACPI read/write config out of range\n");
        return AE_ERROR;
    }

    void *ptr = ((uint8_t *)magenta_pci_tbl.ecam) + offset + reg;
    if (write) {
        switch (width) {
            case 8:
...
static mx_status_t acpi_probe_ecam(void) {
    ...
    magenta_pci_tbl.ecam_size = size_per_bus * num_buses;
    mx_status_t ret = mx_mmap_device_memory(root_resource_handle, base, magenta_pci_tbl.ecam_size,
                                                MX_CACHE_POLICY_UNCACHED_DEVICE,
                                                (uintptr_t *)&magenta_pci_tbl.ecam);
    if (ret == NO_ERROR && LOCAL_TRACE) {
        printf("ACPI: Found PCIe and mapped at %p.\n", magenta_pci_tbl.ecam);
    }
...
static mx_status_t acpi_probe_legacy_pci(void) {
    ...
    if (vendor_id != 0xFFFF) {
        magenta_pci_tbl.has_legacy = true;
...
static ACPI_STATUS AcpiOsReadWritePciConfiguration(
    ...
    //
    // TODO(cja): If ACPI opts to never read or write the PCI config
    // space then we won't do any probing. Unknown whether this is a problem
    // yet.
    if (!magenta_pci_tbl.pci_probed) {
        ...
        magenta_pci_tbl.pci_probed = true;
    }
    ...
    if (magenta_pci_tbl.ecam != NULL) {
        status = acpi_pci_ecam_cfg_rw(PciId, Register, value32, Width, Write);
    } else if (magenta_pci_tbl.has_legacy) {
#if __x86_64__
        status = acpi_pci_x86_pio_cfg_rw(PciId, Register, value32, Width, Write);
#endif
    }
